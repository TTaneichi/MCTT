...
/* -------------------------------------------------------------------
* @brief	Medain Filterのqsort用比較関数
* @param	a	比較対象1(unsigned shortを入れる)
* @param	b	比較対象2(unsigned shortを入れる)
* @retval   比較結果
...
int Cmp(const void *a, const void *b)
{
    ...
    if(*(unsigned short int *)a > *(unsigned short int *)b) return(1);
    return(0);      //if(*(unsigned short int *)a == *(unsigned short int *)b)
}
/* -------------------------------------------------------------------
* @brief	Medain Filterのqsort用比較関数
* @param	a	比較対象1(intを入れる)
* @param	b	比較対象2(intを入れる)
* @retval   比較結果
* @date     2021/03/30	T.Taneichi	流用
 --------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------
* @brief	配列をソートして返す。出力配列はメモリ確保しておくこと
* 			ttsuji（SPC（FPGAエミュレータ）の処理にあわせたもの）
* @param	val			入力データ（未ソート配列)
* @param	ret_val		出力データ(ソート済み配列)
* @param	num			配列要素数
* @retval   なし
* @date     2021/03/30	T.Taneichi	流用
 --------------------------------------------------------------------- */
void quick_sort(const int *val , int * ret_val , const int num)
{
	for (int i = 0; i < num; i++){
		ret_val[i] = val[i];
	}

	qsort(ret_val, num, sizeof(int), IntCmp);
...
double Radians(const double dDegree)
{
	return dDegree * PI / 180.0;
}
// Radian->Degree変換

double Degrees(const double dRadian)
{
...
*/
unsigned short int round(int iVal)
{
    if(iVal > 65535){
        return(65535);
    }else if(iVal <= 0){
        ...
        return((unsigned short int)iVal);
    }
}
//---------------------------------------------------------------------------
/*
